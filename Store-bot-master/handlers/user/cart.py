...
async def process_phone(message: Message, state: FSMContext):
    ...
    await CheckoutState.next()
    await message.answer('Турар жойингизни локатсиясини ташанг.',
                            reply_markup=geo())
...
async def process_locate(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        geoloc = str("https://www.google.com/search?q=%s+%s" % (message.location.latitude, message.location.longitude))
    await confirm(message)
    await CheckoutState.next()
...
async def process_confirm(message: Message, state: FSMContext):
    ...
    if enough_money:
        ...
        async with state.proxy() as data:
            ...
            await message.answer(f'ОК! Сизнинг буюртмангиз тайёр 🚀\nИсм: <b>' + data['name'] + '</b>\nМанзил: <b>' + data['address'] + '</b>\nЛакатсия: <b>' + {geoloc} + '</b>',
                                 reply_markup=markup)
