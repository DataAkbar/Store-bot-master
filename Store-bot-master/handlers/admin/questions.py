    ...
    if len(questions) == 0:
        ...
        for cid, question in questions:
            ...
            markup.add(InlineKeyboardButton(
                'Жавоб бериш', callback_data=question_cb.new(cid=cid, action='answer')))
...
async def process_answer(query: CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await query.message.answer('Жавоб ёзинг.', reply_markup=ReplyKeyboardRemove())
...
async def process_submit(message: Message, state: FSMContext):
    ...
    await message.answer('Жавобингизни хато ёзмаганингизга ишонч ҳосил қилинг.', reply_markup=submit_markup())
...
async def process_send_answer(message: Message, state: FSMContext):
    await message.answer('Бекор этилди!', reply_markup=ReplyKeyboardRemove())
    await state.finish()
